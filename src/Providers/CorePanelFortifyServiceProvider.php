...
final class CorePanelFortifyServiceProvider extends ServiceProvider
{
    ...
    {
        Fortify::createUsersUsing($this->actionClass('create_user', CreatesNewUsers::class, CreateNewUser::class));
        Fortify::updateUserProfileInformationUsing($this->actionClass('update_profile', UpdatesUserProfileInformation::class, UpdateUserProfileInformation::class));
        Fortify::updateUserPasswordsUsing($this->actionClass('update_password', UpdatesUserPasswords::class, UpdateUserPassword::class));
        Fortify::resetUserPasswordsUsing($this->actionClass('reset_password', ResetsUserPasswords::class, ResetUserPassword::class));
        ...
        Fortify::authenticateUsing(function (Request $request): ?Authenticatable {
            ...
            if (
                ...
                && $user->getRelationValue('microsoftAccount') !== null
            ) {
     ...
     */
    private function actionClass(string $key, string $contract, string $default): string
    {
        $class = config("core-panel.auth.actions.{$key}", $default);

        if (! is_string($class) || ! class_exists($class) || ! is_a($class, $contract, true)) {
            throw new InvalidArgumentException(sprintf(
                'CorePanel Fortify action [core-panel.auth.actions.%s] must be a class implementing [%s].',
                $key,
                $contract,
            ));
        }
