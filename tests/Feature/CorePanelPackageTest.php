...
it('preloads the active locale before mounting the publishable inertia app', function (): void {
    ...
    expect($contents)->toContain("import { I18n, i18nVue } from 'laravel-vue-i18n'")
        ->and($contents)->toContain("let currentAppName = 'CorePanel'")
        ->and($contents)->toContain('function resolveAppName(props: Record<string, unknown>): string {')
        ->and($contents)->toContain('const value = props.appName')
        ...
        ->and($contents)->toContain('document.documentElement.dataset.appName = currentAppName')
...
it('renders the publishable logs tabs through the shared table builder surface', function (): void {
    foreach ([
        ...
        $expectation = expect($contents)->toContain(
            ...
            ->toContain("import LogBadge from '@core-panel/pages/Admin/Logs/components/LogBadge.vue'")
            ->toContain('<LogBadge');
...
it('ships scaffold linting, formatting and ci workflow configuration', function (): void {
    ...
    $updateChangelogWorkflow = file_get_contents(__DIR__.'/../../../../.github/workflows/update-changelog.yml');
    ...
    $generateReleaseNotesScript = file_get_contents(__DIR__.'/../../../../.github/scripts/generate_release_notes.sh');
    ...
    expect($tsconfig['compilerOptions']['strict'])->toBeTrue()
        ...
        ->and($workflow)->toContain('ready_for_review')
        ...
        ->and($updateChangelogWorkflow)->toContain('types:')
        ->and($updateChangelogWorkflow)->toContain('- released')
        ->and($updateChangelogWorkflow)->toContain('stefanzweifel/changelog-updater-action@v1')
        ->and($updateChangelogWorkflow)->toContain('stefanzweifel/git-auto-commit-action@v7')
        ->and($updateChangelogWorkflow)->toContain("commit_message: '📝 Docs: Update CHANGELOG'")
        ->and($updateChangelogWorkflow)->toContain('file_pattern: CHANGELOG.md')
        ->and($splitWorkflow)->toContain('workflow_run:')
        ->and($splitWorkflow)->toContain('workflows:')
        ->and($splitWorkflow)->toContain('- Release')
        ->and($splitWorkflow)->toContain('- completed')
        ->and($splitWorkflow)->toContain("if: github.event_name == 'workflow_dispatch' || github.event.workflow_run.conclusion == 'success'")
        ->and($splitWorkflow)->toContain('needs: split-branches')
        ->and($splitWorkflow)->toContain('SPLIT_REPO_TOKEN')
        ->and($splitWorkflow)->toContain('persist-credentials: false')
        ->and($splitWorkflow)->toContain('fetch-tags: true')
        ...
        ->and($splitWorkflow)->toContain("ref: \${{ github.event_name == 'workflow_run' && github.event.workflow_run.head_branch || github.ref_name }}")
        ->and($splitWorkflow)->toContain('Missing SPLIT_REPO_TOKEN secret')
        ->and($splitWorkflow)->toContain('PUSH_URL="https://${PUSH_USER}:${SPLIT_REPO_TOKEN}@github.com/${{ matrix.target_repo }}.git"')
        ->and($splitWorkflow)->toContain('RELEASE_TAG="$(git tag --points-at HEAD | grep -E')
        ->and($splitWorkflow)->toContain('No semver tag found on HEAD. Skipping tag split.')
        ...
        ->and($splitWorkflow)->toContain('gh release view "${RELEASE_TAG}"')
        ->and($splitWorkflow)->toContain('--repo "${GITHUB_REPOSITORY}"')
        ->and($splitWorkflow)->toContain('gh release edit "${RELEASE_TAG}"')
        ->and($splitWorkflow)->toContain('gh release create "${RELEASE_TAG}"')
        ->and($splitWorkflow)->toContain('--notes-file "${RELEASE_NOTES_FILE}"')
        ->and($splitWorkflow)->not->toContain('git push "${PUSH_URL}" "split-${{ matrix.package_name }}":main --force'."\n".'                  git push "${PUSH_URL}" "${RELEASE_TAG}" --force')
        ->and($releaseWorkflow)->toContain("RELEASE_COMMIT_PATTERN: '^🚀 Release:( (major|minor|patch|v?[0-9]+\\.[0-9]+\\.[0-9]+))?$'")
        ->and($releaseWorkflow)->toContain('default: patch')
        ->and($releaseWorkflow)->toContain('type: choice')
        ->and($releaseWorkflow)->toContain('description: Explicit release version (overrides bump)')
        ->and($releaseWorkflow)->toContain("INPUT_VERSION: \${{ github.event.inputs.version || '' }}")
        ->and($releaseWorkflow)->toContain('echo "requested_version=${INPUT_VERSION#v}" >> "$GITHUB_OUTPUT"')
        ->and($releaseWorkflow)->toContain('echo "bump_type=${INPUT_BUMP}" >> "$GITHUB_OUTPUT"')
        ->and($releaseWorkflow)->toContain("REQUESTED_VERSION=\"\$(printf '%s\\n' \"\$COMMIT_MESSAGE\" | sed -En 's/^🚀 Release: (v?[0-9]+\\.[0-9]+\\.[0-9]+)\$/\\1/p')\"")
        ...
        ->and($releaseWorkflow)->toContain('if [ "${SHOULD_RELEASE}" = "true" ] && [ -n "${REQUESTED_VERSION}" ]; then')
        ...
        ->and($releaseWorkflow)->toContain('name: Check release state')
        ->and($releaseWorkflow)->toContain('git rev-parse "refs/tags/${RELEASE_VERSION}"')
        ->and($releaseWorkflow)->toContain('gh release view "${RELEASE_VERSION}"')
        ->and($releaseWorkflow)->toContain('name: Generate release notes')
        ->and($releaseWorkflow)->toContain('chmod +x .github/scripts/generate_release_notes.sh')
        ->and($releaseWorkflow)->toContain('RELEASE_VERSION="${RELEASE_VERSION}" .github/scripts/generate_release_notes.sh > /tmp/core-panel-release-notes.md')
        ->and($releaseWorkflow)->toContain("if: steps.workflow_gate.outputs.should_skip != 'true' && (steps.release_gate.outputs.should_release != 'true' || steps.release_state.outputs.tag_exists != 'true')")
        ...
        ->and($releaseWorkflow)->toContain('--notes-file "/tmp/core-panel-release-notes.md"')
        ->and($generateReleaseNotesScript)->toContain('VERSION="${RELEASE_VERSION:-${GITHUB_REF_NAME:-1.0.0}}"')
        ->and($generateReleaseNotesScript)->toContain("grep -Ev '^⚙️ Chore: set version to '")
        ->and($generateReleaseNotesScript)->toContain("grep -Ev '^🚀 Release:'")
        ->and($generateReleaseNotesScript)->toContain('declare -A SECTION_ICONS=')
...
it('ships a vite config that exposes localhost instead of the invalid 0.0.0.0 browser origin', function (): void {
    ...
    expect($contents)->toContain("import path from 'node:path'")
        ...
        ->and($contents)->toContain("fs.existsSync(path.resolve(hostThemePath, 'index.css'))")
        ->and($contents)->toContain("find: '@core-panel/theme/core-panel'")
        ->and($contents)->toContain('replacement: hasCorePanelThemeOverride()')
        ->and($contents)->toContain('? hostThemePath')
        ->and($contents)->toContain(': packageThemePath')
...
it('renders user management with a reference-style datatable shell and a table-only roles tab variant', function (): void {
    ...
    expect($usersTableContents)->not->toContain("key: 'avatar'")
        ...
        ->and($usersTableContents)->not->toContain("key: 'invitation_status'")
        ...
        ->and($usersTableContents)->not->toContain('#cell-invitation_status')
        ...
        ->and($usersTableContents)->toContain('function invitationStatusSeverity(')
        ->and($usersTableContents)->toContain('function invitationActionIcon(')
        ->and($usersTableContents)->toContain('function invitationActionTooltip(')
        ->and($usersTableContents)->toContain("'circle-check-big'")
        ->and($usersTableContents)->toContain("'circle-alert'")
        ->and($usersTableContents)->toContain("'clock-3'")
        ->and($usersTableContents)->toContain("'envelope'")
        ->and($usersTableContents)->toContain('userRoutes.reinvite.url')
        ->and($usersTableContents)->toContain('page-users.invitation_action_hint.')
        ->and($usersTableContents)->toContain("invitationStatus === 'accepted'")
        ->and($usersTableContents)->toContain('requiresPasswordSetup ||')
        ...
        ->and($usersTableContents)->toContain('class="cp-users-tab cp-datatable"')
        ...
        ->and($userGroupsTabContents)->toContain("import TableBuilderDataTable from '@core-panel/components/TableBuilder/DataTable.vue'")
        ->and($userGroupsTabContents)->toContain('action-column-width="8.25rem"')
        ->and($userGroupsTabContents)->toContain("mode: 'local'")
        ->and($rolesOverviewContents)->toContain("import TableBuilderDataTable from '@core-panel/components/TableBuilder/DataTable.vue'")
        ->and($rolesOverviewContents)->toContain('<TableBuilderDataTable')
        ->and($rolesOverviewContents)->toContain("mode: 'local'")
        ...
        ->and($dataTableContents)->toContain('function syncStickyHeadMetrics(): void {')
        ...
        ->and($dataTableContents)->toContain('class="cp-datatable__sticky-head"')
        ->and($dataTableContents)->toContain('class="cp-datatable__sticky-header-row"')
        ->and($dataTableContents)->toContain('cp-datatable__sticky-header-cell--selection')
        ->and($dataTableContents)->toContain('cp-datatable__sticky-header-icon')
        ->and($dataTableContents)->toContain('class="cp-datatable__table cp-datatable__table--sticky-head"')
        ...
        ->and($packageAppIconContents)->toContain("'chevron-up': ChevronUp")
        ->and($packageAppIconContents)->toContain('sitemap: Workflow')
        ->and($appIconContents)->toContain('Columns3')
        ->and($appIconContents)->toContain("'chevron-up': ChevronUp")
        ->and($appIconContents)->toContain('columns: Columns3')
        ->and($appIconContents)->toContain('sitemap: Workflow')
        ...
        ->and($datatableThemeContents)->toContain('--cp-datatable-sticky-head-height: 0px;')
        ->and($datatableThemeContents)->toContain('.cp-datatable__sticky-head {')
        ->and($datatableThemeContents)->toContain('top: var(--cp-page-header-height, 0px);')
        ->and($datatableThemeContents)->toContain('.cp-datatable__sticky-header-row {')
        ->and($datatableThemeContents)->toContain('.cp-datatable__sticky-header-cell--sortable {')
        ...
        ->and($datatableThemeContents)->toContain('.cp-datatable__table--sticky-head .p-datatable-thead {')
        ->and($datatableThemeContents)->toContain('.cp-datatable__table .cp-datatable__actions-header .p-column-header-content {')
        ->and($datatableThemeContents)->toContain('min-height: 3.85rem;')
        ...
        ->and($datatableThemeContents)->toContain('.core-panel-dark .cp-datatable__sticky-head {')
        ...
        ->and($userGroupsTabContents)->toContain('class="cp-user-groups-tab cp-datatable"')
        ->and($userGroupsTabContents)->toContain('<TableBuilderDataTable')
        ...
        ->and($userGroupsTabContents)->toContain("mode: 'local'")
        ->and($userGroupsTabContents)->toContain('surface-class="cp-user-groups-tab__surface"')
        ...
        ->and($rolesOverviewContents)->toContain('<TableBuilderDataTable')
        ->and($rolesOverviewContents)->toContain('surface-class="cp-roles-panel__surface"')
        ...
        ->and($rolesOverviewContents)->toContain("localSortType: 'number'")
        ->and($rolesOverviewContents)->toContain("localSortType: 'date'")
        ->and($rolesOverviewContents)->not->toContain('{{ $t(\'page-roles.new_role\') }}')
        ->and($rolesOverviewContents)->not->toContain('{{ $t(\'page-roles.resync\') }}')
        ->and($rolesOverviewContents)->toContain('openEditRoleDialog(row as RoleRecord)')
        ->and($rolesOverviewContents)->toContain('confirmDeleteRole(row as RoleRecord)');
});
...
it('uses wayfinder-driven user management endpoints in the user pages', function (): void {
    ...
    expect($index)->toContain("import TabsRenderer from '@core-panel/components/TabBuilder/TabsRenderer.vue'")
        ...
        ->and($show)->toContain("import UserFormDialog from '@core-panel/pages/Admin/Users/components/UserFormDialog.vue'")
        ...
        ->and($avatarUpload)->not->toContain("import userAvatarRoutes from '@/routes/core-panel/users/avatar/index'")
...
it('surfaces api token management inside the settings workspace', function (): void {
    ...
    expect($settingsIndex)->toContain("import ApiSettingsTab from '@core-panel/pages/Admin/Settings/components/ApiSettingsTab.vue'")
        ...
        ->and($apiSettingsTab)->toContain("import ApiTokenManager from '@core-panel/pages/Admin/ApiTokens/components/ApiTokenManager.vue'")
...
it('ships the consolidated developer area with tabbed activity, authentication, and log views', function (): void {
    ...
    expect($routes)->toContain("'logs.php'")
        ...
        ->and($activityTab)->toContain("import LogUserAvatar from '@core-panel/pages/Admin/Logs/components/LogUserAvatar.vue'")
        ->and($activityTab)->toContain('row.causerAvatarUrl ?? null')
        ->and($activityDetail)->toContain("import LogUserAvatar from '@core-panel/pages/Admin/Logs/components/LogUserAvatar.vue'")
        ...
        ->and($authenticationTab)->toContain("import LogUserAvatar from '@core-panel/pages/Admin/Logs/components/LogUserAvatar.vue'")
        ...
        ->and($authenticationTab)->toContain("import AuthenticationLogDetail from '@core-panel/pages/Admin/Logs/components/AuthenticationLogDetail.vue'")
        ->and($authenticationDetail)->toContain("import LogUserAvatar from '@core-panel/pages/Admin/Logs/components/LogUserAvatar.vue'")
...
it('ships a dedicated developer workspace with route inspection and swagger-backed docs', function (): void {
    ...
    expect($routes)->toContain("'developer.php'")
        ...
        ->and($page)->toContain("import RouteListTab from '@core-panel/pages/Admin/Developer/components/RouteListTab.vue'")
...
it('ships locale switching assets and shared locale scaffolding', function (): void {
    ...
    expect($switcher)->toContain("import locale from '@/routes/locale'")
        ...
        ->and($switcher)->toContain('v-if="options.length > 1"')
        ...
        ->and($appHeader)->toContain('page.props.locale?.supported ?? []')
        ->and($appHeader)->toContain('v-if="supportedLocales.length > 1"')
        ->and($appHeader)->toContain('I18n.getSharedInstance().setOptions({')
